...
export async function GET(request: NextRequest) {
  ...
  const excludeId =
    excludeRaw && /^\d+$/.test(excludeRaw.trim())
      ? Number.parseInt(excludeRaw.trim(), 10)
      : undefined;

  try {
    await sequelize.authenticate();
    await BlogPost.sync();
    const existing = await BlogPost.findOne({ where: { slug: normalized } });
    const takenByOther =
    ...
    const suggestion = await allocateUniqueBlogSlug(title, normalized, excludeId);
    ...
    console.error('blog slug-available:', error);
